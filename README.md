...
> （摘要 `380dad61…`，改一处即校验失败）；**阶段验收 5 项全部通过**。
> 见 §11H。
>
...
> （闸门见 §11H.8）。
>
> 命令见第 2 章，架构见第 3 章，AI API 数据格式见第 9 章，自适应干扰闭环见第 8 章，
...
python verify_v4.py                       # 端到端验收：18 章（模块/不变式/旧实验逐位复现/AI发现码/证据链/压力测试/资源管理/优化参考/契约冻结/版本号/产物）
python -m unittest discover -s tests -v   # 全量 474 项（其中 1 项为条件跳过：冻结摘要一致时无需触发严格模式）
...
python evaluate_resource_management.py --acceptance   # 只跑阶段验收 5 项
...
│   ├── tasks.py                    任务队列（未知对象不得提前建任务；状态机；逐类型×状态分布）
│   ├── scheduling.py ★             三个规则基线 + 模板方法（_classify_due/_duplicate_rule 共用）
│   ├── optimization.py ★           非学习优化参考（显式预测模型 / 声明式目标 / 计算预算 /
│   │                                 规范最优性声明白名单 / 帕累托前沿）
│   ├── closed_loop.py ★            闭环编排（世界→融合→通信→调度→执行→记账）
│   ├── acceptance.py ★             阶段验收 5 项（含"改未来不改当前规划"的运行时对照）
...
│
├── docs/ ★
│   ├── data_contract.md            运行隔离与产物流水契约
│   ├── change_report.md            一致性验收阶段的修改记录（E3/E4 等）
│   ├── resource_management.md      资源管理设计说明（§1–§12）
...
**约束版没能达成约束目标（d=0.07），综合收益也低于普通 DQN。这是一个负结果，如实记录。**
...
### 11.5 核心发现：违反率对 λ **非单调**，朴素对偶上升在本环境会发散
...
**能确认成立的**：
1. 机制本身正确——双 critic 正常训练、λ 单调演化、掩码与代价口径一致；
2. **`λ = 0` 时精确退化为普通 DQN**，两者可平滑对比；
3. 存在一个**窄而真实的 λ 有效窗口**（约 0.92~0.96），此时违反率 0.066 < 0.07，
   说明「用拉格朗日约束把违反率压到 7% 以下」在原理上是可行的。

**改进方向**（已列入第 16 章后续工作）：把**能量**也纳入约束（而不是只当作状态）、
改用信任域类方法（CPO）、用 `Q_c` 的期望而非实测违反率做对偶上升、
对 λ 加阻尼/动量、或在 λ 上做一维线搜索而不是梯度上升。

> **答辩/论文里应当这样表述**：「我们实现了拉格朗日约束 DQN 分支并验证了其机制正确，
> 但在本环境的硬能量约束下朴素对偶上升不收敛，约束版未能在训练预算内达成目标违反率；
> 我们给出了失效机理与改进方向。」**不要**宣称约束版优于普通 DQN。
...
| 学习算法接入 | — | — | — | — | — | **未接入**（阶段验收 5 项已通过，闸门见 §11H.8） |
...
* **大阶段一（资源管理）不接入任何旧实验路径**：新增模块只在
  ...
  `evaluate_resource_management.py` 三个入口被调用；历史入口的源码扫描
  仍为空（`verify_v4.py` §15「历史实验入口未引用 resource_management」）。
...
* ✅ **阶段验收 5 项全部通过**（多节点执行真实生效 / 资源不超支 / 信息不越权 /
  任务队列可追溯 / 规则与优化参考可复现），清单与逐项证据落在
...
大阶段一的闸门是「阶段验收 5 项」，**已全部通过**（§11H.8）。
因此下面这条线可以开始，但**不要求**学习方法必须胜出，也**不要求**
规则方法必须失败：

1. **接入学习算法（大阶段二的入口）**：把 `resource_management` 的观测/队列/
   执行器当作固定环境，学"提交什么任务"的策略。进入前必须先补齐：
   * 让 `estimate_quality` 维度**真的对调度敏感**（把共享/刷新的收益接进
     资源约束，即修掉 §11H.8 第 4 条那个建模缺口）——
     否则学到的策略无法在"信息新鲜度"上体现价值；
   * 打开概率检测/虚警或随机化场景，使多种子**真的产生差异**，
     否则任何"提升"都无法与噪声区分（§11H.8 第 3 条）；
   * 保留三个规则基线与两个优化参考为**可复现对照**，且所有方法继续共用
     同一观测/队列/执行器；
2. **给"从未被服务的服务类别"单独加项**：修掉规则调度饿死 `share` 的问题
   （§11H.8 第 5 条）。这不是为了让规则"赢"，而是当前规则本身有明确缺陷；
3. **跨 tick 资源预留**：放开"执行器只支持立即执行"的限制
   （`reserved` 机制已就位），使每节点每 tick 可以排多条任务；
   这同时会改变"完成度由服务上限封顶"的现状，必须重跑基线与契约验收；
4. **跨节点航迹关联**：把各节点各自上报的同一真值航迹合并成全局 ID，
   需要先解决信息边界问题（§11H.11 第 5 条）；
5. **统计意义上的方法比较**：在 (1) 的前置条件满足后，
